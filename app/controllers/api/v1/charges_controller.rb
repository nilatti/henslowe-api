...
class ChargesController < ApiController
  ...
  def create_checkout_session
    ...
    session = Stripe::Checkout::Session.create({
      ...
      cancel_url: "#{ENV.fetch('FRONTEND_URL', 'https://henslowescloud.com')}#{return_to || '/checkout'}",
      customer: user.stripe_customer_id,
      success_url: "#{ENV.fetch('FRONTEND_URL', 'https://henslowescloud.com')}#{return_to || '/success'}",
      mode: 'subscription',
      line_items: [{
        quantity: 1,
        price: params[:price]
  ...
  def update_payment_info
    ...
    session = Stripe::Checkout::Session.create({
      ...
      success_url: "#{ENV.fetch('FRONTEND_URL', 'https://henslowescloud.com')}/account",
      cancel_url: "#{ENV.fetch('FRONTEND_URL', 'https://henslowescloud.com')}/account",
      })
      render json: {stripeUrl: session.url}
  end
