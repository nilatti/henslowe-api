...
RSpec.describe 'Characters API' do
  ...
  describe 'GET api/plays/:play_id/characters' do
    before {
      get "/api/v1/plays/#{play.id}/characters", params: {play_id: play.id}, headers: authenticated_header(user)
    }
  ...
  describe 'POST /plays/:play_id/characters' do
    ...
    context 'when request attributes are valid' do
      before { post "/api/v1/plays/#{play.id}/characters", params: valid_attributes, as: :json, headers: authenticated_header(user) }
    ...
    context 'when an invalid request' do
      before { post "/api/v1/plays/#{play.id}/characters", params: { character: { age: 'Baby', play_id: play.id } }, as: :json, headers: authenticated_header(user) }
